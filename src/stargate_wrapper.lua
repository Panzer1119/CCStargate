...
while true do
    ...
    local event, sid, msg, ptc = os.pullEvent()
	if (event == "rednet_message") then
		if (ptc == PROTOCOL) then
			...
			--print("Client used wrong protocol!")
			sleep(0.05)
			rednet.send(sid, nil, ptc)
		end
	elseif (forwardEvents and (event == "sgDialIn" or event == "sgDialOut" or event == "sgChevronEngaged" or event == "sgStargateStateChange" or event == "sgIrisStateChange" or event == "sgMessageReceived")) then
		sleep(0.05)
		for i = 0, #clients do
			local client = clients[i]
			local data = {event=event, p1=sid, p2=msg, p3=ptc}
			--print("sending " .. textutils.serialise(data))
			if (client ~= nil) then
				rednet.send(client.id, data, PROTOCOL)
				--print("sent to " .. client.id)
			end
		end
	end
end
